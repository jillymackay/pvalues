...
# This plot renders the distribition for n2p for J groups and N obs per group. 
# Effects observed in the sample that are greater that eta >.04 will be statistically significant
  ...
  df2 <- J*(N - 1)
  Ntot = N*J
  ncp = Ntot/(1/eta_pop-1) # non centrality parameter
  ...
  ggplot |> 
    aes(data.frame(c()))+
    stat_function(eta_pop_dist(seq))
  

mock <- data.frame(x =seq(0.00000000001, 1, 0.0001))



mock |> 
  mutate(x = eta_pop_dist(x)) |> 
  ggplot(aes(x)) + 
  geom_density()
